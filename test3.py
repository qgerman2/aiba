...
# Verify no characters were changed or dropped
joined = "".join(phrases)
if joined != transcript:
    print("WARNING: Ollama output does not match transcript exactly.")
    print(f"Expected length: {len(transcript)}")
    print(f"Got length:      {len(joined)}")
    if len(joined) < len(transcript):
        print(f"Missing text: {transcript[len(joined):]}")
    phrases = split_with_regex(transcript)
    print("Fell back to regex splitting.")
...
# Helper: expand multi-character segments into one character per line
...
# Save character-level timestamps: one character per line
